							...
							if any(str(Ag_resi.get_resname()) == x for x in aa_list):  ### Only consider amino acids
								for Ag_atom in Ag_resi:
									...
									if atom_distance <= contact_cutoff and Ab_atom_id[0] != 'H' and Ag_atom_id[0] != 'H':
										if (27 <= int(Ab_resi_id[1]) <= 38) or (56 <= int(Ab_resi_id[1]) <= 65) or (105 <= int(Ab_resi_id[1]) <= 117):
											CDR_contacts += 1
										else:
											FR_contacts += 1
...
path = '../Script1/imgt_all_clean/'

# Specify the summary file
data = pd.read_csv('../Script1/Summary_all.tsv', sep='\t', header=0)
thera_data = pd.read_csv('../Script1/Thera_all.tsv', sep='\t', header=0)
...
pdb_list = data['pdb'].tolist()  # Writing all pdb IDs to a list
