...
def main(summary_data_path: Path, cluster_path: Path, output_path: Path):
	...
	with open(cluster_path) as a_file:
		...
		for line in a_file:
			...
			# If the pdb entry is found on the line this specific cluster id is added to the
...
if __name__ == '__main__':
	...
	parser.add_argument("-s", default=Path('test/work_dir/Summary_all_sorted.fea.zst'), type=Path, help="Path to Summary_all_sorted.fea.zst")
	parser.add_argument("-c", default=Path('test/work_dir/Cluster.txt'), type=Path, help="Path to Cluster.txt")
	parser.add_argument("-o", default=Path('.'), type=Path, help="Path to write output to")
	args = parser.parse_args()
	main(args.s, args.c, args.o)
