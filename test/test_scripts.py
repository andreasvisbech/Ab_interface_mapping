...
class TestScripts:
	...
	def test_script2(self, tmp_path):
		...
		main(Path("test/work_dir/Summary_all_sorted.parquet"), Path("test/work_dir/Cluster.txt"), Path(tmp_path))
	...
	def test_script3(self, tmp_path):
		...
		main(Path('test/work_dir/Summary_all_sorted_nonredundant_index.csv'), Path("."), Path(tmp_path))
